...
export interface Root2 { // this is the interface we make for the movies. its meant to be a template for our response from a http request
  id: string;
  title: string;
...
@Component({
  selector: 'my-app', // use this if you want to load this page somewhere else
  templateUrl: './app.component.html',
  styleUrls: ['./app.component.css'],
})
export class AppComponent implements OnInit {
  name = 'Isaac Ojigho';
  movies: Root2[]; // give our interface a name
  movie2: any;

  constructor(private httpClient: HttpClient) {}

  private url = 'https://ghibliapi.vercel.app/films'; // http url
  ngOnInit(): void {
    this.httpClient.get(this.url).subscribe( // http request and subscription
      (response) => {
        this.movies = response as Root2[]; // store response from api call in response
        console.log('**', this.movies[0]);
        this.getTitle(this.movies[0]);
      },
      (error) => {
        console.log(error);
      }
